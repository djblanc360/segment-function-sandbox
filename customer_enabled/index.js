...
fs.readFile('./destination-payloads/customer_enabled.json', 'utf8', (err, data) => {
    if (err) {
      console.error(er/r);
      return;
    }
...
async function onIdentify(event, settings) {
	console.log('onIdentify', event);
	// rename traits to properties to recycle tracking code
	if (event.traits) {
...
const generateCommonProps = async (event, domain, headers) => {
	...
	var properties = {
		...
		created_at: event.properties.created_at,
		...
		id: event.properties.id.toString(),
		...
		tax_exempt: event.properties.tax_exempt.toString(),
		updated_at: event.properties.updated_at,
