...
function getTotalQuantity(items) {
	...
	console.log('getTotalQuantity items', items);
	let total = 0;
	items.forEach(item => {
		total += item.quantity;
	});
	console.log('getTotalQuantity total', total);
	return total;
}
...
const handleGatewayGranular = async (receiptJson) => {
	...
	let card_brand = order_transactions?.charges?.data[0]?.payment_method_details?.card?.brand;
	let card_description = order_transactions?.charges?.data[0]?.payment_method_details?.card?.description;
	...
	if (receiptJson.toLowerCase().includes('paypal')) {
		...
		card_wallet = order_transactions?.charges?.data[0]?.payment_method_details?.card?.wallet;
		if (card_wallet === null) {
			card_wallet = order_transactions?.charges?.data[0]?.payment_method_details?.card?.brand;
		}
	}
...
const generateCommonProps = async (event, domain, headers) => {
	...
	*/
	...
	var properties = {
		...
		processed_at: convertToUnixTime(event.properties.processed_at),
...
const generateItemProps = async (order, lineItems, domain, headers) => {
	...
	const lineItemProps = await Promise.all(
		lineItems.map(async item => {
			...
			let totalQuantity = getTotalQuantity(item.quantity);
			...
			let totalShipping = 0; // should be 0 for individual items

			let subtotalPrice = parseFloat(item.price) * parseFloat(item.quantity) - parseFloat(item.total_discounts);
