 ...
 async function onTrack(event, settings) {
	...
	if (event.event === 'refund_item') {
		...
		const hardId = (event) => {
			...
			const key = email ? 'registered' : id ? 'registered' : 'phone_id';
...
const getTotalQuantity = items => {
	// NOT USED FOR SINGLE ITEM refund
...
const generateCommonProps = async (event, domain, headers) => {
	...
	let gateway = '';
	if (event.properties.order.refunds.transactions[0].gateway !== undefined) {
		gateway = event.properties.order.refunds.transactions[0].gateway;
	}
	...
	var properties = {
		...
		created_at: convertToUnixTime(event.properties.order.refunds.created_at),
		...
		processed_at: convertToUnixTime(event.properties.order.refunds.processed_at),
		processed_method:
			event.properties.order.refunds.processing_method ?? 'direct',
		refund_id: event.properties.order.id,
...
const generateItemProps = async (order, item, domain, headers) => {
	// let product_id = order.properties.id;
	...
	let totalTax = parseFloat(order.total_tax) / parseFloat(totalQuantity);
	console.log('totalTax', totalTax);
	let totalLineItemsPrice = parseFloat(item.price) * parseFloat(item.quantity) - (parseFloat(item.total_discount) / parseFloat(totalQuantity));
	console.log('totalLineItemsPrice', totalLineItemsPrice);
	let subtotalLineItemsPrice = totalLineItemsPrice - totalTax;
	console.log('subtotalLineItemsPrice', subtotalLineItemsPrice);
	...
	properties.subtotal_price = removeDecimalIfZero(totalLineItemsPrice * -1.0);
	properties.title = item.title;
    properties.total_line_items_price = removeDecimalIfZero(subtotalLineItemsPrice * -1.0);
	properties.total_price = removeDecimalIfZero(totalLineItemsPrice * -1.0);
	properties.total_tax = removeDecimalIfZero(totalTax * -1.0) ?? 0;
