async function onTrack(event, settings) {
	if (event.event === 'customer_disabled') {
		const eventName = 'customer_disabled';
		const { domain, key, secret, token } =
			credentials[event.properties.shopDomain];
		// console.log('domain', domain, 'key', key, 'secret', secret, 'token', token);
...
async function onIdentify(event, settings) {
	...
	const auth = `${key}:${secret}`;
	const endpoint =
		'https://api.exponea.com/track/v2/projects/' + token + '/customers';
