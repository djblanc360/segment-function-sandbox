...
fs.readFile('./json-sources/exchange_item.json', 'utf8', (err, data) => {
    if (err) {
      console.error(er/r);
      return;
    }
 ...
 */
 ...
 async function onTrack(event, settings) {
	...
	const emailId = event.properties.order.customer.email;

	if (event.event === 'exchange_item') {
		...
		const payload = {
			...
			timestamp: properties.updated_at.toString()
		};
...
const getTotalQuantity = items => {
	// NOT USED FOR SINGLE ITEM exchange
...
const generateCommonProps = async (event, domain, headers) => {
	...
	var properties = {
        ...
        removeDecimalIfZero(event.properties.order.subtotal_price_set.presentment_money.amount),
		confirmed: event.properties.order.confirmed,
		created_at: event.properties.order.created_at,
		...
		processed_method: event.properties.order.refunds.processing_method ?? 'direct',
